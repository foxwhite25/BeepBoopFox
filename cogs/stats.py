...
class Stats(commands.Cog):
    ...
    async def register_command(self, ctx):
        ...
        log.info(f'{message.created_at}: {message.author} 在 {destination} 运行 {message.content}')
        async with self._batch_lock:
    ...
    async def about(self, ctx: commands.Context):
        ...
        embed = qq.Embed(
            title=f'机器人版本 {revision[0].short_id}',
        ...
        embed.add_field(name=f'成员:\n{total_members} ')
        embed.add_field(name=f'子频道:\n{text + voice} ')

        memory_usage = self.process.memory_full_info().uss / 1024 ** 2
        cpu_usage = self.process.cpu_percent() / psutil.cpu_count()
        embed.add_field(name=f'进程:\n{memory_usage:.2f} MiB\n{cpu_usage:.2f}% CPU')

        version = pkg_resources.get_distribution('qq.py').version
        embed.add_field(name=f'频道:\n {guilds}')
        embed.add_field(name=f'运行指令数:\n {sum(self.bot.command_stats.values())}')
        embed.add_field(name=f'正常运行时间:\n{self.get_bot_uptime(brief=True)}')
        embed.add_field(name=f'用 qq.py v{version} 制作')
    ...
    async def show_guild_stats(self, ctx):
        ...
        embed = qq.Embed(title='频道命令统计', colour=qq.Colour.blurple())
    ...
    async def bothealth(self, ctx):
        ...
        embed = qq.Embed(title='机器人健康报告', colour=HEALTHY)
    ...
    async def gateway(self, ctx):
        ...
        builder = [
            f'RESUME 总计: {sum(resumes.values())}',
            f'IDENTIFY 总计: {total_identifies}'
        ]
        ...
        for shard_id, shard in self.bot.shards.items():
            ...
            if stats:
                builder.append(f'分片 ID {shard_id}: {badge} ({", ".join(stats)})')
            else:
                builder.append(f'分片 ID {shard_id}: {badge}')
        ...
        embed = qq.Embed(colour=colour, title='Gateway (过去 24 小时)')
        for n in builder:
            embed.add_field(name=f"\n{n}")
        embed.add_field(name=f'{issues} 警告')
        await ctx.reply(embed=embed)
